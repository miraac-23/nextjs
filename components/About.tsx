        ...
        <div className="grid gap-10 lg:grid-cols-[1.3fr_1fr]">
          <Reveal className="space-y-5">
            {profile.about.map((p, i) => (
              <p key={i} className="text-[15.5px] leading-relaxed text-fg2">
                {p}
              </p>
            ))}
            ...
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {[
                ...
                >
                  <div className="text-xs uppercase tracking-wider text-fg4">{item.k}</div>
                  <div className="mt-1 font-medium text-fg">{item.v}</div>
                </div>
              ))}
            </div>
          </Reveal>
              ...
              <div className="relative space-y-6 pl-6">
                ...
                {education.map((e) => (
                  <div key={e.degree} className="relative">
                    <span className="absolute -left-[22px] top-1.5 h-3 w-3 rounded-full border-2 border-accent bg-page" />
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-accent">
                      {e.period}
                    </div>
                    <div className="mt-0.5 text-sm font-semibold text-fg">{e.degree}</div>
                    <div className="text-sm text-fg3">{e.school}</div>
                    <div className="text-xs text-fg4">{e.detail}</div>
                  </div>
                ))}
              </div>
            </div>
