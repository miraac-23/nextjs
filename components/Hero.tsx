      ...
      <div className="container-x relative">
        <div className="grid items-center gap-14 lg:grid-cols-[1.15fr_0.85fr]">
          ...
          <div className="animate-fade-up">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-line/10 bg-surface/5 px-4 py-1.5 text-xs font-medium text-fg2">
            ...
            <p className="mt-5 max-w-xl text-lg leading-relaxed text-fg2">
              <span className="font-semibold text-fg">{profile.role}</span>{' '}
              <span className="text-fg3">— {profile.tagline}</span>
            </p>

            <p className="mt-4 max-w-xl text-[15px] leading-relaxed text-fg3">
              {profile.summary}
            </p>
              ...
              {socials.map((s) => {
                ...
                return (
                  <a
                    ...
                    rel="noreferrer"
                    aria-label={s.label}
                    className="group grid h-11 w-11 place-items-center rounded-xl border border-line/10 bg-surface/5 text-fg2 transition-all duration-300 hover:-translate-y-1 hover:border-accent/50 hover:text-accent-soft"
                  >
            ...
            <div className="relative animate-float">
              <div className="glass overflow-hidden rounded-[2rem] p-6">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-mono text-[11px] text-fg4">profile.ts</span>
                </div>

                <pre className="mt-5 overflow-hidden font-mono text-[12.5px] leading-relaxed text-fg2">
                  <code>
                ...
                <div className="mt-6 grid grid-cols-2 gap-3">
                  {stats.map((s) => (
                    <div
                      key={s.label}
                      className="rounded-2xl border border-line/10 bg-surface/[0.03] p-4 text-center"
                    >
                      <div className="font-display text-2xl font-bold gradient-text">{s.value}</div>
                      <div className="mt-1 text-[11px] uppercase tracking-wider text-fg3">
                        {s.label}
                      </div>
                    </div>
                  ))}
